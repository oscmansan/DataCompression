...
def cond_probs(pair_count):
    d = defaultdict(lambda: [])
    for p in pair_count:
        x = p[0][0] # fist character
        y = p[0][1] # second character
        w = p[1]    # count
        d[x].append((y,w))
    ...
    for c in d.keys():
        cs, ws = zip(*d[c])
        cs = list(cs)
        ps = list(map(lambda x: x/sum(ws),ws))
        d[c] = (cs,ps)
...
class Entropy:
    def __init__(self,txt):
        ...
        for i in range(self.n):
            ai = txt[i]
            aj = txt[(i+1)%self.n] # circular text
            count[ai] += 1
            pair_count[ai+aj] += 1

        self.letters = list(count.keys())
        self.pairs = list(pair_count.keys())
        self.probs = list(map(lambda x: float(x)/self.n, count.values()))
        self.pair_probs = list(map(lambda x: float(x)/(self.n-1), pair_count.values()))
        self.cprobs = cond_probs(pair_count.items())

    def random_text(self):
        text = ''
        for _ in range(self.n):
            text += choices(self.letters,self.probs)[0]
        return text

    def random_text_joint(self):
        text = choices(self.letters,self.probs)[0] # first character
        for _ in range(self.n-1):
            p = self.cprobs[text[-1]]
            text += choices(*p)[0]
        return text
    ...
    def joint_entropy(self):
        H = 0
        for p in self.pair_probs:
            H += p*log(1/p,2)
        return H

    def conditional_entropy1(self,ltr):
        H = 0
        for p in self.cprobs[ltr][1]:
            H += p*log(1/p,2)
        return H

    def conditional_entropy(self):
        H = 0
        for a,p in zip(self.letters,self.probs):
            H += p*self.conditional_entropy1(a)
        return H

def random_text(txt):
    e = Entropy(txt)
    return e.random_text()

def random_text_joint(txt):
    e = Entropy(txt)
    return e.random_text_joint()
...
def joint_entropy(txt):
    e = Entropy(txt)
    return e.joint_entropy()

def conditional_entropy1(txt,ltr):
    e = Entropy(txt)
    return e.conditional_entropy1(ltr)
...
with open('../data/moby_dick.txt','r') as f:
    ...
    rnd_txt = random_text(txt)
    rnd_txt_joint = random_text_joint(txt)

    h1 = entropy(txt)
    h2 = entropy(rnd_txt)
    print(abs(h1-h2))

    h3 = joint_entropy(txt)
    h4 = joint_entropy(rnd_txt_joint)
    print(abs(h3-h4))

    h5 = conditional_entropy(txt)
    h6 = conditional_entropy(rnd_txt_joint)
    print(abs(h5-h6))
