...
def weighted_choice(values,weights):
    total = 0
    cum_weights = []
    for w in weights: #TODO: compute this only once
        total += w
...
class Entropy:
    ...
    def random_text(self):
        ...
        for _ in range(len(self.txt)):
            res += weighted_choice(list(d.keys()),self.probs)

        return res

    def random_text_joint(self):
        res = weighted_choice(list(self.count.keys()),self.probs) # first character
        for _ in range(len(self.txt)-1):
            p = self.cprobs[res[-1]]
            res += weighted_choice(*p)
